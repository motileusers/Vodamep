...
{
    internal class CmReportSerializer
    {
        public CmReport Deserialize(Stream stream)
        {
        ...
        public CmReport Deserialize(byte[] data)
        {
            ...
            if (isJson)
            {
                ...
                r = CmReport.Parser.ParseJson(json);
        ...
        public CmReport DeserializeFile(string file)
        {
            if (!File.Exists(file))
                return null;
        ...
        public void WriteToFile(CmReport report, string filename, bool asJson, bool compressed = true)
        {
        ...
        public string WriteToPath(CmReport report, string path, bool asJson, bool compressed = true)
        {
            var filename = Path.Combine(path, new CmReportFilenameHandler().GetFileName(report, asJson, compressed));

            WriteToFile(report, filename, asJson, compressed);
            return filename;
        }

        public MemoryStream WriteToStream(CmReport report, bool asJson, bool compressed = true)
        {
            ...
            if (compressed)
            {
                var filename = new CmReportFilenameHandler().GetFileName(report, asJson, false);
