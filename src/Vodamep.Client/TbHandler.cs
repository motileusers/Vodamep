...
{
    public class TbHandler : HandlerBase
    {
        ...
        public override void PackRandom(PackRandomArgs args)
        {
            ...
            var r = TbDataGenerator.Instance.CreateTbReport(args.InstitutionId, year, month, args.Persons, args.Staffs, args.AddActivities);
        ...
        protected override void ValidateSingleFile(ValidateArgs args)
        {
            ...
            var formatter = new TbReportValidationResultFormatter(ResultFormatterTemplate.Text, args.IgnoreWarnings);
            var message = formatter.Format(report, result);

            Console.WriteLine(message);
        }

        private TbReport ReadReport(string file)
        {
            try
            {
                return TbReport.ReadFile(file);
            }
            catch (Exception ex)
            {
