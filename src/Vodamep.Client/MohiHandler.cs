...
namespace Vodamep.Client
{
    public class MohiHandler : HandlerBase
    {
        ...
        protected override void ValidateSingleFile(ValidateArgs args)
        {
            ...
            var formatter = new MohiReportValidationResultFormatter(ResultFormatterTemplate.Text, args.IgnoreWarnings);
            var message = formatter.Format(report, result);

            Console.WriteLine(message);
        }

        private MohiReport ReadReport(string file)
        {
            try
            {
                return MohiReport.ReadFile(file);
            }
            catch (Exception ex)
            {
